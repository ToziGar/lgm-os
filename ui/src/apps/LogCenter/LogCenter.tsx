...
const SYSTEM_MSGS: { level: LogLevel; service: string; message: string }[] = [
  { level: 'info',     service: 'kernel',      message: 'CPU temperature: 52°C' },
  { level: 'info',     service: 'networkd',    message: 'Link eth0 is UP (1000Mbps full duplex)' },
  { level: 'info',     service: 'systemd',     message: 'Unit nginx.service started successfully' },
  { level: 'warning',  service: 'kernel',      message: 'Memory pressure event triggered (80% used)' },
  { level: 'info',     service: 'cron',        message: 'Running scheduled task: nightly-backup' },
  { level: 'error',    service: 'mariadbd',    message: 'Too many connections (max: 100)' },
  { level: 'info',     service: 'avahi',       message: 'Registering mDNS service: lgm-nas-01.local' },
  { level: 'debug',    service: 'sshd',        message: 'Received disconnect from 192.168.1.200' },
  { level: 'info',     service: 'smbd',        message: 'Connection established from 192.168.1.200' },
  { level: 'warning',  service: 'disk',        message: '/dev/sda1 usage at 42% (growing fast)' },
  { level: 'critical', service: 'backup',      message: 'Backup job failed: destination unreachable' },
  { level: 'info',     service: 'update',      message: 'Package update check completed: 3 updates available' },
  { level: 'info',     service: 'nginx',       message: 'GET /api/status 200 12ms' },
  { level: 'warning',  service: 'sshd',        message: 'Failed password for root from 45.33.32.156 port 54321' },
  { level: 'error',    service: 'nginx',       message: 'Connect() failed (111: Connection refused)' },
  { level: 'info',     service: 'lgm-ui',      message: 'Session started for user admin from 192.168.1.100' },
];
